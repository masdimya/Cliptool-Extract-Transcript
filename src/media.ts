...
export const downloadArgs = (outputTemplate: string, url: string): string[] => [
  '--no-playlist', '--newline', '--progress', '-f', 'bv*[height<=720]+ba/b[height<=720]',
...
export async function downloadVideo(ytdlp: string, url: string, directory: string, signal?: AbortSignal): Promise<string> {
  await run(ytdlp, downloadArgs(join(directory, 'video.%(ext)s'), url), { signal, inherit: true })
